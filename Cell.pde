class Cell {
  ...
  void updateModulators() {
    ...
    modulators[3] = noise(dna.genes[27]*1000); // NOISE_X
    modulators[4] = noise(dna.genes[28]*1000); // NOISE_Y
  ...
  void updateVelocity() {
    ...
    float vx = modulator(modulators[3], -dna.genes[25], dna.genes[25]);
    float vy = modulator(modulators[4], -dna.genes[25], dna.genes[25]);
    ...
    //Update twistAngle
    //float twist = radians(map(twistMod, 1, 0, twist_start, twist_end));
